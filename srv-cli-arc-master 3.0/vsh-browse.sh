...
#content_dir=$(sed -e '/^directory [A-Za-z0-9]*/!d' -e 's/^directory //' -e 's/\r$//' $arch)
#arg=$(get_absolute_path $arg)

#if [ "$arg" = "$content_dir" ];then
   ...
   #         echo "Suppression par chemin absolu de "$arg
    #        echo "directory "$directory""
            ...
            # Si l'argument est un fichier
                ...
                # Check si le fichier est vide
                if [[ "$file_size" = "0" ]];then 
		#echo $(echo $file_to_delete | awk '{print $1}')" est vide."
                echo ""
